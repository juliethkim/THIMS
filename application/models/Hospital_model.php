...
class Hospital_model extends CI_Model
{
    ...
    {
        $this->db->select('BaseTbl.Id, BaseTbl.name, BaseTbl.category, BaseTbl.ownership, BaseTbl.region, BaseTbl.district');
        $this->db->from('hospitals as BaseTbl');
      
        if(!empty($searchText)) {
            $likeCriteria = "(BaseTbl.name  LIKE '%".$searchText."%'
                            OR  BaseTbl.category  LIKE '%".$searchText."%'
                            OR  BaseTbl.ownership  LIKE '%".$searchText."%')";
  ...
  /*  function checkEmailExists($email, $userId = 0)
    ...
    {
        $this->db->trans_start();
        $this->db->insert('hospitals', $hospitalInfo);
    ...
    {
        $this->db->select('hospitalId, name, category, ownership, region, district');
        $this->db->from('hospitals');
        $this->db->where('isDeleted', 0);
     ...
     * @param number $userId : This is user id
     */
    function editHospital($hospitalInfo, $hospitalId)
    {
        $this->db->where('hospitalId', $hospitalId);
        $this->db->update('hospitals', $hospitalInfo);
     ...
     */
    function deleteHospital($hospitalId, $hospitalInfo)
    {
        $this->db->where('hospitalId', $hospitalId);
        $this->db->update('hospitals', $hospitalInfo);
